...
class Auth:
  ...
  def sign_in_with_email_and_password(self, email, password, timeout=3):
    ...
    request_object = requests.post(request_ref, headers=headers, data=data, timeout=timeout)
  ...
  def sign_in_with_custom_token(self, token, timeout=3):
  ...
  def refresh(self, refresh_token, timeout=3):
  ...
  def get_account_info(self, id_token, timeout=3):
  ...
  def send_email_verification(self, id_token, timeout=3):
  ...
  def send_password_reset_email(self, email, timeout=3):
    request_ref = "https://www.googleapis.com/identitytoolkit/v3/relyingparty/getOobConfirmationCode?key={0}".format(self.api_key)
    headers = {
      "content-type": "application/json; charset=UTF-8"
    }
  ...
  def verify_password_reset_code(self, reset_code, new_password, timeout=3):
  ...
  def create_user_with_email_and_password(self, email, password, timeout=3):
...
class Database:
  ...
  def get(self, token=None, json_kwargs={}, timeout=3):
    ...
    # do request
    request_object = self.requests.get(request_ref, headers=headers, timeout=timeout)
    raise_detailed_error(request_object)
  ...
  def push(self, data, token=None, json_kwargs={}, timeout=3):
    request_ref = self.check_token(self.database_url, self.path, token)
    self.path = ""
    headers = self.build_headers(token)
    request_object = self.requests.post(request_ref, headers=headers, data=json.dumps(data, **json_kwargs).encode("utf-8"), timeout=timeout)
    raise_detailed_error(request_object)
    return request_object.json()

  def set(self, data, token=None, json_kwargs={}, timeout=3):
    request_ref = self.check_token(self.database_url, self.path, token)
    self.path = ""
    headers = self.build_headers(token)
    request_object = self.requests.put(request_ref, headers=headers, data=json.dumps(data, **json_kwargs).encode("utf-8"), timeout=timeout)
    raise_detailed_error(request_object)
    return request_object.json()

  def update(self, data, token=None, json_kwargs={}, timeout=3):
    request_ref = self.check_token(self.database_url, self.path, token)
    self.path = ""
    headers = self.build_headers(token)
    request_object = self.requests.patch(request_ref, headers=headers, data=json.dumps(data, **json_kwargs).encode("utf-8"), timeout=timeout)
    raise_detailed_error(request_object)
    return request_object.json()

  def remove(self, token=None, timeout=3):
    request_ref = self.check_token(self.database_url, self.path, token)
    self.path = ""
    headers = self.build_headers(token)
    request_object = self.requests.delete(request_ref, headers=headers, timeout=timeout)
    raise_detailed_error(request_object)
    return request_object.json()
